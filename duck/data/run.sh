...
while true; do
    ...
    bashio::log.info "https://www.duckdns.org/update?domains=${DOMAINS}&token=${TOKEN}&ip=${IPV4}&txt=${TXT}&verbose=true"
    if answer="$(curl -s "https://www.duckdns.org/update?domains=${DOMAINS}&token=${TOKEN}&ip=${IPV4}&txt=${TXT}&verbose=true")"; then
